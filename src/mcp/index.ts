...
async function main() {
    ...
    async ({ repoPath, commitHashStart, commitHashEnd }) => {
      try {
        const output = execSync(
          `git diff ${commitHashStart} ${commitHashEnd}`,
          {
            cwd: repoPath,
            encoding: 'utf8',
          },
  ...
  server.registerTool(
    ...
    async ({ repoPath, commitHashStart, commitHashEnd, summary }) => {
      try {
        db.prepare(
          `INSERT INTO session_logs (repo_path, commit_hash_start, commit_hash_end, summary)
           VALUES (?, ?, ?, ?)`,
        ).run(repoPath, commitHashStart, commitHashEnd, summary);
    ...
    async ({ repoPath, commitHash }) => {
      try {
        const row = db
          ...
          .get(repoPath, commitHash, commitHash) as SessionLog | undefined;
    ...
    async ({ repoPath }) => {
      try {
        const rows = db
          ...
          .all(repoPath, repoPath, repoPath) as PendingCommit[];
...
if (process.argv[1]?.endsWith('mcp/index.js')) {
  startMcpServer().catch((err) => {
    process.stderr.write(
      `[gitstoria-mcp] fatal: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    process.exit(1);
  });
}
