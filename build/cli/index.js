...
program
    ...
    if (!existsSync(dbPath)) {
        process.stderr.write(`[gitstoria] warning: database not found at ${dbPath} — run "gitstoria init" first\n`);
        process.exit(0);
    }
    try {
        const db = getOrInitDb();
        db.prepare('INSERT INTO pending_commits (repo_path, commit_hash) VALUES (?, ?)').run(opts.repo, opts.hash);
    }
    catch (err) {
        process.stderr.write(`[gitstoria] error recording commit: ${err instanceof Error ? err.message : String(err)}\n`);
        process.exit(1);
    }
});
program
